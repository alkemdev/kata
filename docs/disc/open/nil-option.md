...
## Context
KataScript already has `nil` as a runtime value (`Value::Nil`). The type system design introduces `Opt[T]` as a builtin type — a plain enum defined in KS itself (`enum Opt[T] { Some(T), None }`). The question is whether these coexist, and if so, how.

This is Hoare's "billion-dollar mistake" territory. Dynamically-typed languages traditionally use `nil`/`null`/`None` everywhere, which leads to null-reference errors. Statically-typed languages like Rust use `Option<T>` to make absence explicit at the type level. KataScript is dynamically typed but aspires to structural typing via `kind` — the answer here affects how much null-safety the language can offer.
...
### Option B: `Opt[T]` only
No `nil` value. Absence is always `Opt.None`, presence is `Opt.Some(v)`. Pattern matching required to extract values.
...
### Option C: Both — `nil` as sugar for `Opt.None`
`nil` exists as a literal but is semantically `Opt.None`. A variable of type `T` cannot be `nil`; only `Opt[T]` can. `nil` is sugar that infers the `Opt` wrapper.
...
In a dynamically-typed language, `Opt` is less compelling than in Rust because there's no compiler to enforce exhaustive matching. Without static type checking, `Opt.Some(v)` vs `nil` is just ceremony — the runtime error moves from "nil reference" to "forgot to unwrap Opt."

Key constraint: `Opt[T]` is a regular enum defined in KS (`enum Opt[T] { Some(T), None }`), not a runtime primitive. This makes Option C (nil-as-sugar) harder — the runtime would need to know about a user-defined type to desugar `nil` into it.
...
- Python `None` — universal nil, no safety
